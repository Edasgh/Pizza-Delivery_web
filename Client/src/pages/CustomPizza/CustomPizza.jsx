...
const CustomPizza = () => {
    ...
    const addToCart = async (e) => {
        ...
        if (pizzaToppingsVal.length !== 0) {
            ...
            if (token) {
                try {
                    const response = await axios.post(`${BACKEND_BASE_URL}/api/product/cart/addToCart`, {
            ...
            toast.error("Toppings field is empty!")
        }
    ...
    return (
      <>
        ...
        <div style={{ width: "70%", margin: "auto", marginBottom: "2rem" }}>
          ...
          <form className="form" id="addProduct" onSubmit={addToCart}>
            ...
            <div className="option" id="variant-option-container">
              {pizzaCrustVal && pizzaCrustVal?.variants?.length !== 0 ? (
                ...
                <>
                  ...
                  >
                    Choose a Pizza Crust to view available variants...
                  </p>
                  <br />
                </>
              )}
                ...
                {pizzaToppings.map((topping) => (
                  <div className="option" key={topping._id}>
                    <input
                      ...
                      onChange={(e) => {
                        handleChooseToppings(e, topping);
                      }}
                    />
                    <label htmlFor={`topping-${topping._id}`}>
                      {topping.name}({topping.category})
                    </label>
                    <input
                      type="text"
                      name="topping-price"
                      value={`Price : ${topping.price}rs`}
                      style={{ padding: ".3rem" }}
                      readOnly
                    />
                  </div>
